...
const DUMMY_MEALS = [
  ...
  },
  {
    ...
    groupName: 'Drinks',
    description:
      "Chocolate, Peanut Butter,Cream & Crushed Reese’s",
    price: 9.5,
  },
  {
    id: "m15",
    ...
    groupName: 'Drinks',
    description:
      "Homemade Blueberry Syrup, Cream Cheese, Biscuit Crumbs",
    price: 9.5,
  },
];

function AvailableMeals(props) {
  const mealsList = DUMMY_MEALS.filter(x => x.groupName === props.selectedTab).map((meal) => (
    <MealItem
      id={meal.id}
      key={meal.id}
      name={meal.name}
